...
## The eight cause families, and the signal each actually leaves

**connection_pool_exhaustion** — `acquire_mean_ms` far above the underlying query
time, `pending_peak_count` well above zero, `active_peak_count` pinned at
`pool_max_count`, throughput flat while latency climbs linearly with users. The
tell is that latency rises but the database itself is not busy.

**thread_pool_saturation** — `tomcat.threads.busy` at `threads.max`, requests
queueing before they reach any application code. Distinguishable from pool
exhaustion by *where* the wait is: thread saturation shows a large gap between
request arrival and controller entry, while pool exhaustion shows the wait inside
`acquire`.
