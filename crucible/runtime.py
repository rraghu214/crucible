...
def metered(worker: Skill) -> Skill:
    ...
    which is the only place ``crucible.telemetry`` reads from.
    """
...
class AgentRuntime:
    ...
    def __init__(self, root: Path | None = None) -> None:
        ...
        self.root = root or Path(os.getenv("CRUCIBLE_DATA_DIR", str(Path.home() / ".crucible")))
    ...
    def _skills(self) -> "SkillManager | None":
        """Markdown skills, discovered once from CRUCIBLE_SKILLS_DIR.
        ...
        root = os.getenv("CRUCIBLE_SKILLS_DIR")
        if not root:
            return None
    ...
    async def run(self, *, prompt: str | None, scope: MemoryScope | None, llm: TextLLM | None = None,
        ...
        surface_request = {"max_tokens": int(os.getenv("CRUCIBLE_SURFACE_MAX_TOKENS", "4000")),
                           "agent": "crucible_compose_surface"}
        ...
        if not os.getenv("CRUCIBLE_SANDBOX_ROOT"):
        ...
        if not os.getenv("CRUCIBLE_WORKSPACE"):
        ...
        async def planning_llm(planning_prompt: str, system: str) -> dict[str, Any]:
            ...
            with call_site(f"plan_{planner_calls}", "planner") as site:
                try:
                    ...
                    setattr(error, "_crucible_planner_meter", site.result_fields())
                    raise
            return {**result, "_crucible_planner_meter": site.result_fields()}
        ...
        planner: Any = GeneralAgentPlanner(
            ...
            max_nodes=int(os.getenv("CRUCIBLE_MAX_GRAPH_NODES", "32")),
            max_new_tasks=int(os.getenv("CRUCIBLE_MAX_FRONTIER", "4")),
            initial_evidence=initial_evidence, unavailable=unavailable,
            max_repeat_failures=int(os.getenv("CRUCIBLE_MAX_REPEAT_FAILURES", "4")),
            skills=self._skills(),
        )
        ...
        report = await LiveGraphExecutor(
            ...
            max_workers=int(os.getenv("CRUCIBLE_MAX_WORKERS", "4")),
    ...
    def index_document(self, *, text: str, source_uri: str, scope: MemoryScope, source_author: str) -> dict[str, Any]:
        from crucible.core.memory.chunking import prepare_markdown, semantic_chunks
