...
from crucible.core.live_graph import Deferred, TaskSpec
...
from crucible.workers.context import RunContext
from crucible.workers.parsing import _as_section, _parse_json_array, _parse_json_object, _slug
...
async def run_retriever(ctx: RunContext, task: TaskSpec) -> dict[str, Any]:
    recall_task = TaskSpec(task.id, "memory_recall", {"query": task.input.get("query", ctx.goal)})
    hits = await special.recall(ctx, recall_task, inbound_id=None)
...
async def list_directory(ctx: RunContext, task: TaskSpec) -> dict[str, Any]:
    root = Path(os.environ["CRUCIBLE_SANDBOX_ROOT"]).expanduser().resolve()
...
async def a2a_delegate(ctx: RunContext, task: TaskSpec) -> dict[str, Any]:
    trusted_dir = os.getenv("CRUCIBLE_A2A_TRUSTED_KEYS_DIR")
    allow_unsigned = os.getenv("CRUCIBLE_A2A_ALLOW_UNSIGNED", "0").lower() in {"1", "true", "yes"}
...
async def load_skill(ctx: RunContext, task: TaskSpec) -> dict[str, Any]:
    ...
    if manager is None:
        raise RuntimeError("no skills are configured; set CRUCIBLE_SKILLS_DIR")
...
async def launch_job(ctx: RunContext, task: TaskSpec) -> Deferred:
    ...
    """
    callback_base = os.getenv("CRUCIBLE_CALLBACK_BASE_URL", os.getenv("CRUCIBLE_BASE_URL", ""))
    if not callback_base:
        raise RuntimeError("launch_job needs CRUCIBLE_CALLBACK_BASE_URL or CRUCIBLE_BASE_URL")
    envelope = {
    ...
    token = os.getenv("CRUCIBLE_COMPLETION_TOKEN")
    if token:
