...
from crucible.auth import require_completion, require_control
from crucible.core.memory import MemoryKind, MemoryScope, Principal
...
async def channel_message(
    ...
    """Run any GLC channel envelope through the same Crucible live graph."""
    expected = os.getenv("CRUCIBLE_CHANNEL_BRIDGE_TOKEN", "").strip()
    supplied = (authorization or "").removeprefix("Bearer ").strip()
    if not expected:
        raise HTTPException(503, "CRUCIBLE_CHANNEL_BRIDGE_TOKEN is not configured")
    ...
    from crucible.events import EventEnvelope
    ...
    configured_authority = {
        item.strip()
        for item in os.getenv("CRUCIBLE_CHANNEL_ALLOWED_SIDE_EFFECTS", "").split(",")
        if item.strip()
    }
    ...
    try:
        ...
        result = await request.app.state.runtime.run(
            prompt=prompt,
            scope=MemoryScope(
                os.getenv("CRUCIBLE_CHANNEL_TENANT", "local"),
                "channels",
                principal_id,
                "crucible-channel-agent",
            ),
            llm=lambda prompt, system: gateway_text_llm(request.app, prompt, system),
