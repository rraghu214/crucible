...
`CRUCIBLE_EXEC_CONTAINER=1` runs the same command inside a container instead, which is
...
def allowlist() -> tuple[str, ...]:
    configured = os.getenv("CRUCIBLE_ALLOWED_COMMANDS", "").strip()
    if configured:
...
def run_command(workspace: Workspace, command: str | list[str], *,
    ...
    if os.getenv("CRUCIBLE_EXEC_CONTAINER", "").strip() == "1":
        image = os.getenv("CRUCIBLE_EXEC_IMAGE", "python:3.11-slim")
